...
## 2026-09-22 · 阶段 2 验收后的修订

### 两项排查的结论

**「13 页只有 12 张」不是丢页，是界面没说清楚。** 生成 13 页样本跑了一遍，直接解析 ZIP 的中央目录（不依赖 JSZip）：13 个条目 `_p01`…`_p13` 齐全，提示和按钮都是 13。少的只有缩略图，因为 D26 设了 12 张上限却没告诉用户。现在多一句「只显示前 12 张预览，全部 13 张都在 ZIP 里」，`planThumbnails()` 保证 `shown + hidden === total`。

**「Convert another 看不见」的原因不是文字。** 实测文字对比度本来就达标（浅色 5.17:1 / 深色 6.41:1）。真正的问题是边框只有 1.26:1 / 1.32:1，远低于 WCAG 1.4.11 对控件边界要求的 3:1 —— 看得见字，看不出是按钮。新增 `--border-strong`（3.54:1 / 3.64:1），文字改用 `--fg`（16.8:1 / 15.1:1）。

### 完成了什么

- **对比度**（D34）：新增控件边界令牌；`contrast.test.js` 直接解析 `style.css` 的令牌做断言，改回低对比度会让 CI 红
- **缩略图对账**（D33）：`planThumbnails()` + 说明文字
- **降采样提示带尺寸**：「输出为 3445×4869 像素」；尺寸不一致时改用另一条文案报告最大值
- **分享 / 存到相册**（D29）：仅在 `navigator.canShare({files})` 为真时显示；≤20 张时把全部 JPG 一次交给分享菜单；超限隐藏按钮并说明原因；`navigator.share` 在点击事件里同步调用，文件在转换结束时就备好；AbortError 静默
- **wasm**（D31）：`scripts/copy-pdfjs-assets.js` 在 predev/prebuild 复制进 `public/pdfjs/`，引擎设 `wasmUrl`；不新增依赖
- **后台预加载**（D32）：空闲时预热 pdf.js
- **像素上限决策记录**（D30）
- 测试从 156 增至 **214** 个
...
| 项 | 结果 |
|---|---|
| 13 页 | 提示 13、ZIP 13 条、缩略图 12 张 + 「All 13 are in the ZIP」 |
| A0@300 | 「rendered at 3445×4869 pixels instead」 |
| 分享（模拟支持的设备） | ≤20 张显示按钮、交出 13 个文件带标题；取消静默；真失败给提示；50 张隐藏按钮并说明 |
| 分享（桌面 Chrome 真实环境） | 按钮隐藏，符合预期 |
| 预热 | 用户还没选文件，pdf.js 与 worker 已加载完 |
| 深色模式 | 次要按钮边框 `#707078`、文字 `#ececea`，明确是按钮 |

### 卡在哪里 / 需要 Leo 做的事

1. **那份 13 页中文 PDF 没拿到** —— `fixtures/manual/` 是空的。没有文件就给不出它的确切数字（页面点尺寸、userUnit）。
   已知边界：A4 / Letter 在 72/150/300 三档下**都不会**降采样；A3 要 300 DPI 才触发，A1/A0 在 150 DPI 就触发。
   如果那份文件是 A4 且用 150 DPI 却全页降采样，才是 bug。请把文件放进 `fixtures/manual/`，或直接告诉我它的页面尺寸。
   引擎现在每页附带 `source` 诊断（点尺寸、请求缩放、原始像素、上限），调试页转换后 `console.table` 打印。
2. **分享上限 20 张待真机验证**（D29），阶段 3 在你的 iPhone 上实测后定最终值。
